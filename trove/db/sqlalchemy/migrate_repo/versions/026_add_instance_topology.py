...
instance_topology = Table(
    'instance_topologies',
    meta,
    Column('instance_id', String(36), ForeignKey('instances.id'),
           nullable=False, index=True),
    Column('datastore', String(255), nullable=False),
    Column('topology', Text(), nullable=False),
    Column('created', DateTime()),
    Column('updated', DateTime()),
    Column('deleted', Boolean(), default=0),
    Column('deleted_at', DateTime()),
    # Primary key is datastore/instance id
    PrimaryKeyConstraint('instance_id', 'datastore')
)
...
def upgrade(migrate_engine):
    ...
    create_tables([instance_topology])


def downgrade(migrate_engine):
    meta.bind = migrate_engine
    drop_tables([instance_topology])
