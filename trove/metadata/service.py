...
class MetadataController(wsgi.Controller):
    ...
    def show(req, tenant_id, instance_id, key):
        ...
        if dbmeta.get(key):
            ...
            LOG.info(_('No metadata found for metadata key %s') % key)
    ...
    def create(req, body, tenant_id, instance_id, key):
        ...
        if not result:
            ...
            LOG.info(_('Returned result from metadata lookup: %s') % metadata)
            return wsgi.Result(
                views.MetadataKeyValueView(metadata, req).data(), 400)
    ...
    def edit(req, body, tenant_id, instance_id, key):
        ...
        if key in dbmeta:
            ...
            LOG.info(_('Key: %s not found in database, edit impossible'))
            return wsgi.Result(None, 404)
    ...
    def update(req, body, tenant_id, instance_id, key):
        ...
        if old_key:
            ...
            LOG.info(_('Metadata key: %s could not be found in the database') %
                     key)
            return wsgi.Result(None, 404)
    ...
    def delete(req, tenant_id, instance_id, key):
        ...
        if dbmeta.get(key):
            ...
            LOG.info(_('Metadata key: %s not present in the database') %
                     key)
            return wsgi.Result(None, 404)
